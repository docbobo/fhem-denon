...
DENON_AVR_Parse(@)
{
	...
	if ($msg =~ /^PW(.+)/)
	{
		...
		readingsBulkUpdate($hash, "channelLevelFrontLeft", $volume / 10 - 50);
	}
	elsif ($msg =~ /^CVFR(.+)/){
		my $volume = $1;
		if (length($volume) == 3){
		...
		readingsBulkUpdate($hash, "channelLevelFrontRight", $volume / 10 - 50);
	}
	elsif ($msg eq "CVEND"){
        #CVEND can be ignored. It just shows the end of the CV events	
		Log $ll5, "DENON_AVR_Parse: Ignoring CVEND";	
	}
	elsif ($msg =~ /^MU(.+)/)
	{
	...
	{
		readingsBulkUpdate($hash, "maximumVolume", $1);
	}
	elsif ($msg =~ /^MV(.+)/)
	{
		my $volume = $1;
	...
	{
		readingsBulkUpdate($hash, "videoSelect", $1);
	}
	elsif ($msg =~/^MS(.+)/)
	{
		...
		readingsBulkUpdate($hash, "screenText0", $text);
	}elsif ($msg =~/^NSE/){
	    my $flags = ord(substr($msg,4,1));
		my $text = substr($msg,5);
		readingsBulkUpdate($hash, "screenText".substr($msg,3,1), $text);
		readingsBulkUpdate($hash, "screenFlags".substr($msg,3,1), $flags);
	}
	else 
	{
